...
"desoremote: Add a git remote for matching deso repository"
...
function check_product()
{
    ...
    if (echo -n $1 | grep -q -e "^deso_") ; then
       DESO_BUILD=$(echo -n $1 | sed -e 's/^deso_//g')
       export BUILD_NUMBER=$((date +%s%N ; echo $DESO_BUILD; hostname) | openssl sha1 | sed -e 's/.*=//g; s/ //g' | cut -c1-10)
    else
       DESO_BUILD=
    fi
    export DESO_BUILD
...
function breakfast()
{
    ...
    if [ $# -eq 0 ]; then
        ...
        if [ $? -eq 0 ]; then
            ...
            # This is probably just the DESOLATION model name
            if [ -z "$variant" ]; then
                variant="userdebug"
            fi
...
function lunch()
{
    ...
    if [ $? -ne 0 ]
    then
        # if we can't find a product, try to grab it off the DesolationRom github
        T=$(gettop)
        pushd $T > /dev/null
...
{
    if [ "$OUT" ] ; then
        MODVERSION=$(get_build_var DESO_VERSION)
    ...
    if (adb shell getprop ro.deso.device | grep -q "$DESO_BUILD");
    then
...
EOF
    ...
    return $?
        ...
        echo "The connected device does not appear to be $DESO_BUILD, run away!"
    fi
}
...
function fixup_common_out_dir() {
    ...
    if [ ! -z $DESO_FIXUP_COMMON_OUT ]; then
...
function dopush()
{
    ...
    if (adb shell getprop ro.deso.device | grep -q "$DESO_BUILD") || [ "$FORCE_PUSH" == "true" ];
    then
...
# Add completions
check_bash_version && {
    dirs="sdk/bash_completion vendor/deso/bash_completion"
    for dir in $dirs; do
    if [ -d ${dir} ]; then
